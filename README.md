...
**目标**：验证连续 2 次未参与，第 3 次起进入观察状态，无法投票；管理员可人工解除。

1. 以管理员身份，连续创建 **3 个独立的议题**（A / B / C），全部推进到 **表决期**（SQL 修改时间快速跳过公示期、讨论期）。
2. **议题 A（第 1 次）**：让 user001、user002、user003、user004 都投票，user005 **不投**。
   - 管理员将 A 归档。
   - 此时 user005 的 `consecutiveMiss = 1`，状态仍为 **NORMAL**。
3. **议题 B（第 2 次）**：再次让 user001 ~ user004 投票，user005 还是 **不投**。
   - 管理员将 B 归档。
   - 此时 user005 的 `consecutiveMiss = 2`，状态仍为 **NORMAL**（第 3 次才触发）。
4. **议题 C（第 3 次）**：
   - 管理员将 C 推进到表决期。
   - 登录 **user005** 账号：
     - 顶部出现 **⚠️ 观察状态** 橙色标识
     - 进入议题 C，看不到投票按钮，显示 *"您的账号处于观察状态，无法投票，请联系物业人工解除"*
     - 尝试直接调用 API（用 Postman 或浏览器）`POST /api/topics/{id}/vote`，返回 HTTP 400 + 同样的错误信息
5. ✅ 验证观察状态生效：
   - user005 的确不能参与议题 C
6. **人工解除**：
   - 管理员登录 → 住户管理 → 找到 user005（状态显示 **观察状态**，连续未参与 = 2+，如果 C 已归档则是 3）
   - 点击 **解除观察** 按钮并确认
   - 刷新 user005 页面，观察状态标识消失，状态恢复 **正常**，可继续参与后续议题
7. ✅ 人工解除验证通过。
